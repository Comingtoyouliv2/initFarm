...
/* ─── InterwovenKit Preview Mock ───
   This file is a standalone preview of the production app.
   The real InterwovenKit integration lives in src/main.jsx + src/App.jsx.
   This mock replicates the useInterwovenKit() API for artifact rendering. */
const _IKCtx = createContext(null);
function useInterwovenKit() {
  return useContext(_IKCtx);
}
function _IKProvider({ children }) {
  const [address, setAddress] = useState(null);
  const [username, setUsername] = useState(null);
  const [autoSignEnabled, setAutoSignEnabled] = useState({});
  const [showConnectModal, setShowConnectModal] = useState(false);
  const disconnect = () => { setAddress(null); setUsername(null); setAutoSignEnabled({}); };
  const autoSign = {
    isEnabledByChain: autoSignEnabled,
    isLoading: false,
    enable: async (cid) => setAutoSignEnabled(p => ({ ...p, [cid]: true })),
    disable: async (cid) => setAutoSignEnabled(p => ({ ...p, [cid]: false })),
  };
  const openConnect = () => {
    const addr = "init1" + Array.from({ length: 38 }, () => "0123456789abcdef"[Math.floor(Math.random() * 16)]).join("");
    setAddress(addr);
    setUsername(addr.slice(0, 8) + ".init");
  };
  return (
    <_IKCtx.Provider value={{
      address, initiaAddress: address, hexAddress: address, username,
      isConnected: !!address, isOpen: false,
...
/* ─── App ─── */
/* ─── App (preview wraps with mock provider, production uses src/main.jsx) ─── */
function AppInner() {
